--TEST--
Method overrideing test for DateTime::createFromFormat()
--SKIPIF--
<?php
$required_version = "5.3.1";
$required_func = array("timecop_freeze");
$required_class = array("TimecopOrigDateTime");
$required_method = array(array("TimecopOrigDateTime", "createFromFormat"));
...
foreach ($tests_args as $args) {
    timecop_freeze($dt0);
    $dt1 = call_user_func_array(array("DateTime","createFromFormat"), $args);
    ...
    while (true) {
        ...
        $dt2 = call_user_func_array(array("DateTime","createFromFormat"), $args);
        $dt3 = call_user_func_array(array("TimecopOrigDateTime","createFromFormat"), $args);
        if ($start_time === time()) {
            if ($dt2 && $dt3 && ($dt2->format("c") !== $dt3->format("c"))) {
                printf("DateTime::createFromFormat('%s', '%s') is differ from TimecopOrigDateTime::createFromFormat() : %s !== %s\n",
...
string\(32\) "2010-01-02 (03:04:05|00:00:00)\.654321-08:00"
